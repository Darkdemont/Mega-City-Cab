...
public class DriverServlet extends HttpServlet {
    ...
    private void handleAddDriver(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        // ✅ If username and password are not provided, generate defaults
        if (isNullOrEmpty(username)) {
            username = generateUsername(driverName);
        }
        if (isNullOrEmpty(password)) {
            password = driverLicense; // Default password is the driver license
        }
    ...
    // ✅ Utility method to check for missing parameters
    ...
    // ✅ Generate a unique username from the driver name
